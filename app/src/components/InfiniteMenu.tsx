...
class ArcballControl {
  ...
  private onPointerDownHandler = (e: PointerEvent) => {
  ...
  private onPointerLeaveHandler = () => {
    this.isPointerDown = false;
  };
  private onPointerMoveHandler = (e: PointerEvent) => {
  ...
  constructor(canvas: HTMLCanvasElement, updateCallback: (deltaTime: number) => void) {
    ...
    canvas.addEventListener("pointerdown", this.onPointerDownHandler);
    canvas.addEventListener("pointerup", this.onPointerUpHandler);
    canvas.addEventListener("pointerleave", this.onPointerLeaveHandler);
    canvas.addEventListener("pointermove", this.onPointerMoveHandler);
  ...
  destroy() {
    ...
    this.canvas.removeEventListener("pointerleave", this.onPointerLeaveHandler);
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    onActiveItemChange: (index: number) => void,
    onMovementChange: (isMoving: boolean) => void,
    onInit: ((instance: InfiniteGridMenu) => void) | null = null,
    scale = 1.0
  ) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  private init(onInit: ((instance: InfiniteGridMenu) => void) | null) {
    const gl = this.canvas.getContext("webgl2", { antialias: true, alpha: true });
    if (!gl) {
      console.error("No WebGL 2 context available for InfiniteMenu!");
      return;
    }
    this.gl = gl;
          ...
          new Promise<HTMLImageElement | HTMLCanvasElement>((resolve) => {
            ...
            img.onerror = () => {
              ...
              if (fctx) {
                ...
                fctx.fillText(item.title || "Project", cellSize / 2, cellSize / 2);
              }
              resolve(fb);
            };
            img.src = item.image;
...
export interface InfiniteMenuProps {
  ...
  onItemSelect?: (item: InfiniteMenuItem) => void;
}
      ...
      {activeItem && (
        <>
          <div className={`face-title ${isMoving ? "inactive" : "active"}`}>
            ...
            >
              {activeItem.categories?.[0] || "FEATURED PROJECT"}
            </span>
            <div style={{ color: "#fff", fontWeight: 800 }}>{activeItem.title}</div>
          </div>
          ...
          <div
            ...
            title={activeItem.link ? "Explore project" : "View Case Study"}
